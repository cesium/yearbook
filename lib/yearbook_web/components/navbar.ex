defmodule YearbookWeb.Navbar do
  ...
  def navbar(assigns) do
    ~H"""
    <div class="bg-white flex items-center justify-between w-full h-20 px-4 border-b-2 border-primary">
      ...
      <div class="flex items-center gap-3">
        <%= if assigns[:current_scope] && assigns.current_scope.user do %>
          <.link
            ...
            class="inline-flex items-center justify-center w-32 h-10 font-semibold hover:text-primary transition"
          >
            Settings
          </.link>
          <.link
